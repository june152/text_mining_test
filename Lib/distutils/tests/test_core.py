...
class CoreTestCase(support.EnvironGuard, unittest.TestCase):
    ...
    def cleanup_testfn(self):
        path = crawling.support.TESTFN
        if os.path.isfile(path):
            os.remove(path)
        elif os.path.isdir(path):
            shutil.rmtree(path)

    def write_setup(self, text, path=crawling.support.TESTFN):
        f = open(path, "w")
        try:
            f.write(text)
        finally:
    ...
    def test_run_setup_uses_current_dir(self):
        ...
        os.mkdir(crawling.support.TESTFN)
        setup_py = os.path.join(crawling.support.TESTFN, "setup.py")
        distutils.core.run_setup(

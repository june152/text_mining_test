...
class Test_OpenGL_libs(unittest.TestCase):
    ...
    def test_shell_injection(self):
        result = find_library('; echo Hello shell > ' + crawling.support.TESTFN)
        self.assertFalse(os.path.lexists(crawling.support.TESTFN))
...
class FindLibraryLinux(unittest.TestCase):
    def test_find_on_libpath(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            with crawling.support.EnvironmentVarGuard() as env:
                KEY = 'LD_LIBRARY_PATH'
                if KEY not in env:
                    v = d
                else:
